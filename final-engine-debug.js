...
Quaternion.prototype = {
    ...
    setEuler: function( pitch, yaw, roll ) {
		...
		a[ 0 ] = sinPitch * cosYaw * cosRoll - cosPitch * sinYaw * sinRoll;
		a[ 1 ] = cosPitch * sinYaw * cosRoll + sinPitch * cosYaw * sinRoll;
		a[ 2 ] = cosPitch * cosYaw * sinRoll - sinPitch * sinYaw * cosRoll;
		a[ 3 ] = cosPitch * cosYaw * cosRoll + sinPitch * sinYaw * sinRoll;
                ...
                for ( i = 0; i < l; i++ ) {
                    ...
                    switch ( line[ 0 ] ){
                        ...
                        case 'map_Kd':
                            ...
                            var m = lines[ i ].trim().match( /^map_Kd((\s+((-s [0-9] [0-9] [0-9])|(-o [0-9] [0-9] [0-9])|(-mm [0-9] [0-9])|(-bm [0-9.]+)))*)\s+(\w.+)$/ );
                            if ( !m ) {
                                throw "unsupported map_Kd format: " + JSON.stringify( lines[ i ] ) + ".";
                            }
                            materials[ currentMaterial ].diffuseTexture = baseUrl + m[ m.length - 1 ];
                            break;
                        case 'Ka':
